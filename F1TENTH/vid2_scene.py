...
class Euc(ZoomedScene):
	def construct(self):
		...
		for (i, j), z in np.ndenumerate(im_dist):
			if z == 0.0:
				text_array.append(Tex(str(round(z, 2)), color=WHITE, font_size=20).move_to((-4 + j) * 0.78 * RIGHT + (-4 + i) * 0.78 * DOWN))
			else:
				text_array.append(Tex(str(round(z, 2)), color=BLACK, font_size=20).move_to((-4 + j) * 0.78 * RIGHT + (-4 + i) * 0.78 * DOWN))
...
# contributed by TheoremofBeethoven, www.youtube.com/c/TheoremofBeethoven
	...
	def construct(self):
		...
		self.play(AnimationGroup(frame.animate.shift(1.5 * RIGHT + 0.5 * DOWN), Transform(manim_dist_transform,manim_centerline), lag_ratio=0.2), run_time=4)
		self.wait()
