        ...
        for theta, r in zip(angles, distances):
            ...
            if count < 7:
                ...
                new_calculations = MathTex(
                    ...
                    "\\\\",
                    f"x &= {r:.1f}m\\cos({np.degrees(theta):.1f}^\\circ )",
                    f"= {(r * np.cos(theta)):.1f} m",
                    "\\\\",
                    f"y &= {r:.1f}m\\sin({np.degrees(theta):.1f}^\\circ )",
                    f"={(r * np.sin(theta)):.1f} m",
                    "\\\\",
                    tex_environment="align*",
                    font_size=30,
                )
            ...
            if count >= 7:
                ...
                if count == 7:
                    ...
                    self.play(
                        ...
                        lidar.animate.shift(3 * LEFT).scale(0.35),
                        line.animate.shift(3 * LEFT),
                        FadeIn(grid_group),
                        Transform(line, new_line),
                        cell.animate.set_fill(WHITE, opacity=0.6),
                        FadeIn(new_calculations),
                        run_time=0.5,
                    )
                else:
                    self.play(
                        Transform(line, new_line),
                        cell.animate.set_fill(WHITE, opacity=0.6),
                        TransformMatchingTex(calculations, new_calculations),
                        run_time=0.5,
                    )
                self.wait(0.1)
