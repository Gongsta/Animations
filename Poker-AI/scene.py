...
class KuhnPart1(Scene):
    def construct(self):
        """
        1) Stack 3 cards, face down
        2) Distribute to left, distrute to right
        3) Check, Bet or Fold
        4) Show Sequence of cards winning
        5) Reveal Ace and King, Ace wins
        """
        ...
        self.wait(2)
        self.play(Write(kuhnPoker, run_time=1.5))

        self.wait(1)

        back1 = ImageMobject("../assets/back.png").shift(2 * UP)
        back2 = ImageMobject("../assets/back.png").shift(2 * UP + 0.2 * (RIGHT + DOWN))
        back3 = ImageMobject("../assets/back.png").shift(2 * UP + 0.4 * (RIGHT + DOWN))
        player_text = Text("Player", color=WHITE).shift(2 * DOWN + 3.5 * LEFT)
        opponent_text = Text("Opponent", color=WHITE).shift(2 * DOWN + 3.5 * RIGHT)
        self.play(
            AnimationGroup(
                FadeOut(kuhnPoker),
                AnimationGroup(FadeIn(back1, back2, back3), FadeIn(player_text, opponent_text)),
                lag_ratio=0.7,
            )
        )
        self.play(back3.animate.next_to(player_text, UP))
        self.play(back2.animate.next_to(opponent_text, UP), FadeOut(back1))

        check_text = Text('"Check"', color=WHITE).scale(2)
        bet_text = Text('"Bet"', color=WHITE).scale(2)
        fold_text = Text('"Fold"', color=WHITE).scale(2)
        self.wait(1)
        self.play(FadeIn(check_text))
        # self.play(ReplacementTransform(check_text, bet_text))
        # self.play(ReplacementTransform(bet_text, fold_text))
        self.play(FadeOut(check_text), FadeIn(bet_text))
        self.play(FadeOut(bet_text), FadeIn(fold_text))
        self.wait(1)
        self.play(FadeOut(fold_text))
        self.wait(1)
        ace = ImageMobject("../assets/Ac.png").next_to(player_text, UP)
        queen = ImageMobject("../assets/Qc.png").next_to(opponent_text, UP)
        self.play(FadeIn(ace, queen), FadeOut(back2, back3))
        winner_text = Text("Player wins!", color=WHITE).scale(1.4)
        group = Group(player_text, ace)
        self.wait(1)
        self.play(
            Write(winner_text),
            group.animate.shift(0.2 * UP),
            opponent_text.animate.set_opacity(0.4),
            queen.animate.set_opacity(0.4),
        )
        # self.wait(1)

        # back = ImageMobject('../assets/back.png')
        # self.add(back)
        # self.play(FadeOut(ace, run_time=0.5), FadeOut(king, run_time=0.5), FadeOut(queen, run_time=0.5))
...
class KuhnPart2(MovingCameraScene):
    def construct(self):
        ...
        opponent = Text("OPPONENT", color=WHITE, font_size=34).move_to(
            queen.get_center() + 2.5 * LEFT
        )
        ...
        unknown_2 = unknown.copy().move_to(king.get_center() + 3 * UP)
        self.play(
            self.camera.frame.animate.move_to(qkline.get_center()).set_width(13),
            FadeIn(qkline, you, opponent, unknown_2, king),
        )
        self.wait(0.5)
        self.play(self.camera.frame.animate.move_to(king.get_center() + DOWN).set(width=11))
        ...
        check2 = Tex("Check", color=WHITE, font_size=34).move_to(
            check.get_center() + 0.8 * RIGHT + 1.3 * DOWN
        )

        bet2_line = Line(check.get_center() + 0.1 * DOWN, bet2.get_center(), color=WHITE, buff=0.4)
        check2_line = Line(
            check.get_center() + 0.1 * DOWN, check2.get_center(), color=WHITE, buff=0.4
        )

        call2 = Tex("Call", color=WHITE, font_size=34).move_to(
            bet2.get_center() + LEFT + 1.5 * DOWN
        )
        fold2 = Tex("Fold", color=WHITE, font_size=34).move_to(
            bet2.get_center() + RIGHT + 1.5 * DOWN
        )
        ...
        self.play(
            AnimationGroup(Create(call_line), Write(call), lag_ratio=0.5, run_time=1),
            AnimationGroup(Create(fold_line), Write(fold), lag_ratio=0.5, run_time=1),
            AnimationGroup(Create(bet2_line), Write(bet2), lag_ratio=0.5, run_time=1),
            AnimationGroup(Create(check2_line), Write(check2), lag_ratio=0.5, run_time=1),
            self.camera.frame.animate.move_to(king.get_center() + 3 * DOWN).set(width=13),
        )
        self.wait(1)
        # --- Update to new CFR values ---
        self.play(
            AnimationGroup(Create(call2_line), Write(call2), lag_ratio=0.5, run_time=1),
            AnimationGroup(Create(fold2_line), Write(fold2), lag_ratio=0.5, run_time=1),
        )
        self.wait(0.5)

        ellipse1 = Ellipse(width=check.width + 0.4, height=check.height + 0.4).move_to(
            check.get_center()
        )
        ellipse2 = Ellipse(width=bet.width + 0.4, height=bet.height + 0.4).move_to(bet.get_center())
        ellipse3 = Ellipse(width=call2.width + 0.4, height=call2.height + 0.4).move_to(
            call2.get_center()
        )
        ellipse4 = Ellipse(width=fold2.width + 0.4, height=fold2.height + 0.4).move_to(
            fold2.get_center()
        )
        ...
        self.play(
            self.camera.frame.animate.set(width=23).move_to(king_group.get_center()),
            FadeIn(
                ace_group,
                queen_group,
                ace_card,
...
class KuhnPart3(MovingCameraScene):
    def construct(self):

        queen = ImageMobject("../assets/Qc.png")
        self.play(FadeIn(queen))
        self.play(queen.animate.shift(UP).scale(0.6))
        check = Tex("Check", color=WHITE, font_size=34).move_to(1.5 * LEFT + 1.2 * DOWN)
        bet = Tex("Bet", color=WHITE, font_size=34).move_to(1.5 * RIGHT + 1.2 * DOWN)
        check_line = Line(
            queen.get_center() + 0.7 * DOWN, check.get_center(), color=WHITE, buff=0.5
        )
        bet_line = Line(queen.get_center() + 0.7 * DOWN, bet.get_center(), color=WHITE, buff=0.5)
        self.play(
            AnimationGroup(Create(check_line), Write(check), lag_ratio=0.5, run_time=1),
            AnimationGroup(Create(bet_line), Write(bet), lag_ratio=0.5, run_time=1),
        )
        self.wait(2)

        # self.play(bet_line.animate.set_stroke_width(7), bet.animate.set_font_size(45))
        unknown = ImageMobject("../assets/unknown.png").scale(0.6)

        bet_underlined = Underline(bet)
        bet_underlined.set_stroke(width=2)
        self.play(
            VGroup(check_line, check).animate.set_opacity(0.3),
            queen.animate.set_fill_opacity(0.3),
            self.camera.frame.animate.move_to(bet.get_center()).set(width=10),
        )
        self.wait(1)
        call = Tex("Call", color=WHITE, font_size=34).move_to(bet.get_center() + LEFT + 1.3 * DOWN)
        fold = Tex("Fold", color=WHITE, font_size=34).move_to(bet.get_center() + RIGHT + 1.3 * DOWN)
        ...
        call_line = Line(bet.get_center() + 0.1 * DOWN, call.get_center(), color=WHITE, buff=0.4)
        fold_line = Line(bet.get_center() + 0.1 * DOWN, fold.get_center(), color=WHITE, buff=0.4)
        new_left_fifty = Tex("50\%", color=WHITE, font_size=24).move_to(
            call_line.get_center() + 0.25 * UP + 0.25 * LEFT
        )
        new_right_fifty = Tex("50\%", color=WHITE, font_size=24).move_to(
            fold_line.get_center() + 0.25 * UP + 0.25 * RIGHT
        )
        self.wait(0.5)
        self.play(
            AnimationGroup(Create(call_line), Write(call), lag_ratio=0.5, run_time=1),
        ...
        self.wait(1)
        self.play(Write(new_left_fifty), Write(new_right_fifty))
        self.wait(1)
        ...
        self.play(
            VGroup(new_right_fifty, fold, fold_line).animate.set_opacity(0.3),
            Write(call_underlined),
            Uncreate(bet_underlined),
            self.camera.frame.animate.move_to(call.get_center()).set(width=10),
        )

        king = ImageMobject("../assets/Kc.png").move_to(queen.get_center() + 3 * UP).scale(0.6)
        qkline = Line(king.get_center(), queen.get_center(), color=WHITE, buff=1)

        opponent = Text("OPPONENT", color=WHITE, font_size=34).move_to(
            king.get_center() + 2.5 * LEFT
        )
        you = Text("YOU", color=WHITE, font_size=34).move_to(queen.get_center() + 2 * LEFT)

        unknown_2 = unknown.copy().move_to(queen.get_center() + 3 * UP)
        self.add(unknown_2)
        self.wait(1)
        self.play(
            self.camera.frame.animate.move_to(qkline.get_center()).set_width(13),
            FadeIn(qkline, you, opponent),
        )
        self.play(FadeOut(unknown_2), FadeIn(king))
        ...
        self.play(
            VGroup(call_line, call, new_left_fifty, lose_1).animate.set_opacity(0.3),
            Uncreate(call_underlined),
        )

        self.play(
            Write(fold_underlined),
            VGroup(fold_line, fold, new_right_fifty).animate.set_opacity(1),
        )
        ...
        # --- Update to new CFR values ---
        new_new_left_fifty = (
            Tex("33\%", color=WHITE, font_size=24)
            .move_to(call_line.get_center() + 0.25 * UP + 0.25 * LEFT)
            .set_opacity(0.3)
        )
        new_new_right_fifty = Tex("67\%", color=WHITE, font_size=24).move_to(
            fold_line.get_center() + 0.25 * UP + 0.25 * RIGHT
        )

        self.play(
            AnimationGroup(
                Transform(new_left_fifty, new_new_left_fifty),
                Transform(new_right_fifty, new_new_right_fifty),
                run_time=1,
            )
        )
        ...
        # self.play(VGroup(bet, bet_line).animate.set_opacity(0.3), Uncreate(bet_underlined),
        # )

    #     check_underlined = Underline(check)
    #     check_underlined.set_stroke(width=2)
    #     self.play(Write(check_underlined), VGroup(check, check_line).animate.set_opacity(1),
    ...
    #     queen_group = VGroup(check, check_line, bet, bet_line, call, call_line, check2, check2_line, bet2, bet2_line, call2, call2_line, fold, fold_line, fold2, fold2_line, qkline)

    #     ace_group = queen_group.copy().move_to(queen_group.get_center() + 6 * LEFT)
    #     king_group = queen_group.copy().move_to(queen_group.get_center() + 6 * RIGHT)

    #     ace_card = ImageMobject('../assets/Ac.png').shift(6*LEFT + UP).scale(0.6)
    #     king_card = ImageMobject('../assets/Qc.png').shift(6*RIGHT + UP).scale(0.6)

    #     unknown.move_to(king.get_center())
    #     unknown_ace = unknown.copy().shift(6*LEFT)
    #     unknown_king = unknown.copy().shift(6*RIGHT)
    #     unknown_queen = unknown.copy()

    #     opponent.move_to(unknown_ace.get_center() + 2.5*LEFT)
    #     you.move_to(ace_card.get_center() + 2*LEFT)

    #     self.play(self.camera.frame.animate.set(width=23).move_to(queen_group.get_center()),
    #     FadeIn(ace_group, king_group, ace_card, king_card, unknown_ace, unknown_king, unknown_queen),
    #     FadeOut(new_left_fifty, new_right_fifty, lose_1, lose_2), # TODO: Add the other ones
    #     )


class KuhnSim(Scene):
    def create_card(self, card):
        return ImageMobject(f"../assets/{card}.png")
    ...
    def construct(self):
        ...
        self.wait(2)
        queen = self.create_card("Qc")
        hundred = Text("100%", font_size=100).shift(1.2 * DOWN)
        self.play(FadeIn(queen), FadeIn(hundred))
        self.wait(2)
        ...
        self.play(
            rock.animate.move_to(5 * LEFT + DOWN),
            FadeOut(hundred),
            FadeIn(
                plane,
                line_graph,
                you,
                opp,
                your_score_placeholder,
                opponent_score_placeholder,
                Text("Win Rate over Time", font_size=30).next_to(plane, UP),
                Text("Time", font_size=20).next_to(plane, DOWN).shift(2 * RIGHT),
                Text("Win Rate", font_size=20).next_to(plane, LEFT).shift(2 * UP),
            ),
        )
        self.wait(1)

        for i in range(1, 25):
            group = ["queen", "king", "ace"]
            ...
            if (your_score_tracker.get_value() + opponent_score_tracker.get_value()) == 0:
                ...
                y_values.append(
                    your_score_tracker.get_value()
                    / (your_score_tracker.get_value() + opponent_score_tracker.get_value())
                )
            ...
            your_score_updated = Text(
                str(int(your_score_tracker.get_value())), font_size=80
            ).next_to(you, DOWN)
            opponent_score_updated = Text(
                str(int(opponent_score_tracker.get_value())), font_size=80
            ).next_to(opp, DOWN)
            ...
            self.play(
                AnimationGroup(
                    your_score_placeholder.animate.become(your_score_updated),
                    opponent_score_placeholder.animate.become(opponent_score_updated),
                    run_time=0.2,
                )
            )
            # self.wait(0.2)
            ...
            if i <= 3:
                ...
                run_time = 0.05
            self.play(
                line_graph.animate.become(
                    new_plane.plot_line_graph(
                        x_values=x_values,
                        y_values=y_values,
                        line_color=GOLD_E,
                        vertex_dot_radius=0,
                ...
                plane.animate.become(new_plane),
                run_time=run_time,
            )
...
# config.background_color = BLACK
class RegretMatching(Scene):
    def construct(self):
        name = Tex(r"$\text{Regret Matching}$", color=WHITE, font_size=70).shift(2.5 * UP)
        regret_matching = Tex(
            r"$\sigma_{i}^{T+1}(I, a) = \begin{cases} \frac{R_i^{T, +}(I,a)}{\sum_{a \in A(I)} R_i^{T, +} (I,a) } & \sum_{a \in A(I)} R_i^{T, +} (I,a) > 0\\ \frac{1}{|A(I)|} & \text{otherwise}\end{cases}$"
        )
        self.play(FadeIn(regret_matching, name))
...
class CFRScene(Scene):
    ...
    def construct(self):
        ...
        terminal_histories = cfr.solve(
            method="manim", debug=False
        )  # Get all possible rollouts and use that to build our tree
        tracker = cfr.tracker
...
class NashEquilibriumText(Scene):
    def construct(self):
        ...
        self.play(
            AnimationGroup(
                nashEquilibrium.animate.shift(2 * UP), FadeIn(d, run_time=1.5), lag_ratio=1
            )
        )
        self.wait(2)
...
class bits(Scene):
    def construct(self):
        ...
        bits = Text(
            r"0000000000000000000000000000000000000000000000000", font=MONOLISA_FONT, font_size=28
        )
        self.play(ReplacementTransform(cardsText, bits))
        bits_with_cards = Text(
            r"0010001000000000000000000000000000000000000000100", font=MONOLISA_FONT, font_size=28
        )
...
class RPS(Scene):
    def construct(self):
        ...
        opponentText = Paragraph("Opponent\nChoice", font_size=40, alignment="center").shift(
            2.7 * UP
        )
        arrow = Arrow(ORIGIN, DOWN, buff=0.1, stroke_width=3, color=BLACK).next_to(
            opponentText, DOWN
        )
        self.play(
            Write(opponentText),
            rock.animate.shift(0.5 * DOWN),
            paper.animate.shift(0.5 * DOWN),
            scissors.animate.shift(0.5 * DOWN),
        )
        # oval = Ellipse(width=4, height=8, color=BLACK, fill_opacity=0)
        highlight = (
            ImageMobject("assets/emphasize.png")
            .scale(0.6)
            .stretch(0.92, 1)
            .shift(0.2 * UP)
            .stretch(0.9, 0)
        ...
        scissors2 = (
            ImageMobject("assets/question.png").scale(0.5).shift(4.5 * RIGHT).shift(0.5 * DOWN)
        )
        self.play(
            FadeIn(rock2),
            FadeIn(paper2),
            FadeIn(scissors2),
            group.animate.shift(9 * LEFT),
            FadeOut(scissors),
            FadeOut(paper),
            FadeOut(rock),
        )
...
class RPS2(Scene):
    def construct(self):
        ...
        self.play(
            AnimationGroup(
                FadeIn(rock, scale=0.4),
                FadeIn(paper, scale=0.4),
                FadeIn(scissors, scale=0.4),
                lag_ratio=0.7,
            )
        )
        self.wait(1)
        ...
        line1.add_updater(
            lambda m: m.put_start_and_end_on([min(rock.get_x(), -0.5), 1, 0], x1 + DOWN)
        )
        line2.add_updater(lambda m: m.put_start_and_end_on([paper.get_x(), 1, 0], x2 + DOWN))
        line3.add_updater(
            lambda m: m.put_start_and_end_on([max(scissors.get_x(), 0.5), 1, 0], x3 + DOWN)
        )

        self.play(
            AnimationGroup(
                FadeIn(paper2, scale=0.4),
                FadeIn(scissors2, scale=0.4),
                FadeIn(rock2, scale=0.4),
                lag_ratio=0.7,
            ),
        ...
        self.play(
            AnimationGroup(
                AnimationGroup(rock.animate.shift(4 * RIGHT), scissors.animate.shift(4 * LEFT)),
                FadeIn(q),
                lag_ratio=0.3,
            )
        )
        ...
        self.play(
            AnimationGroup(
                AnimationGroup(FadeOut(line1), FadeOut(line2), FadeOut(line3), FadeOut(q)),
                AnimationGroup(
                    rock2.animate.shift(2.5 * UP),
                    paper2.animate.shift(2.5 * UP),
                    scissors2.animate.shift(2.5 * UP),
                ),
                lag_ratio=0.3,
            )
        )
        ...
        self.wait(2)
        self.play(
            FadeOut(one_third_text),
            FadeOut(one_third_text2),
...
class RPSold(Scene):
    ...
    def construct(self):
        ...
        self.play(
            Transform(rText, youText),
            Transform(pText, vs),
            ReplacementTransform(sText, opponentText),
        )
        ...
        self.play(
            AnimationGroup(Transform(opponentText, blueStart), Write(player_text), lag_ratio=0.6)
        )
        ...
        redScissors = (
            Circle(0.3, color=BLUE).move_to(blueStart.get_center()).shift(3 * RIGHT + 2 * DOWN)
        )
        ...
        line1.add_updater(
            lambda z: (
                z.become(
                    Line(
                        normalize(blueStart1.get_center() - blueStart.get_center()) * 0.3
                        + blueStart.get_center(),
                        normalize(-blueStart1.get_center() + blueStart.get_center()) * 0.3
                        + blueStart1.get_center(),
                    )
                )
                if blueStart.point_at_angle(225 * DEGREES)[0]
                > blueStart1.point_at_angle(45 * DEGREES)[0]
                else None
            )
        )
        line2.add_updater(
            lambda z: (
                z.become(
                    Line(
                        blueStart.point_at_angle(270 * DEGREES),
                        blueStart2.point_at_angle(90 * DEGREES),
                    )
                )
                if blueStart.point_at_angle(270 * DEGREES)[1]
                > blueStart2.point_at_angle(90 * DEGREES)[1]
                else None
            )
        )
        line3.add_updater(
            lambda z: (
                z.become(
                    Line(
                        normalize(blueStart3.get_center() - blueStart.get_center()) * 0.3
                        + blueStart.get_center(),
                        normalize(-blueStart3.get_center() + blueStart.get_center()) * 0.3
                        + blueStart3.get_center(),
                    )
                )
                if blueStart.point_at_angle(315 * DEGREES)[0]
                < blueStart3.point_at_angle(135 * DEGREES)[0]
                else None
            )
        )
        ...
        opponent_text = (
            Tex("You", font_size=36).move_to(redScissors.get_center()).shift(1.5 * RIGHT)
        )
        rockVec = redRock.get_center() - blueStart.get_center()
        scissorsVec = redScissors.get_center() - blueStart.get_center()
        rockText = (
            Tex("Rock", font_size=28)
            .rotate(np.arctan(rockVec[1] / rockVec[0]))
            .move_to(line1.point_from_proportion(0.6) + 0.3 * UP)
        )
        paperText = (
            Tex("Paper", font_size=28)
            .rotate(-PI / 2)
            .move_to(line2.point_from_proportion(0.5) + 0.23 * RIGHT)
        )
        scissorsText = (
            Tex("Scissors", font_size=28)
            .rotate(np.arctan(scissorsVec[1] / scissorsVec[0]))
            .move_to(line3.point_from_proportion(0.6) + 0.3 * UP)
        )
        ...
        self.play(
            AnimationGroup(
                AnimationGroup(
                    Transform(blueStart1, redRPS[0]),
                    Transform(blueStart2, redRPS[1]),
                    Transform(blueStart3, redRPS[2]),
                ),
                AnimationGroup(
                    Write(rockText), Write(paperText), Write(scissorsText), Write(opponent_text)
                ),
                lag_ratio=0.7,
            )
        )
        ...
        redRPSDuplicates = []
        for val in redRPS:
            redRPSDuplicates.append(
                [Circle(0.3, color=BLUE).move_to(val.get_center()) for _ in range(3)]
            )
        ...
        for i in range(3):  # i == 0 -> Rock, i == 1 -> Paper, i == 2 -> Scissors
            ...
            for j in range(3):
                if j == 0:
                    redRPSTransforms[i].append(
                        Transform(
                            redRPSDuplicates[i][0],
                            Circle(0.3, color=GREY)
                            .move_to(redRPS[i].get_center())
                            .shift(LEFT + 2 * DOWN),
                        )
                    )
                elif j == 1:
                    redRPSTransforms[i].append(
                        Transform(
                            redRPSDuplicates[i][1],
                            Circle(0.3, color=GREY).move_to(redRPS[i].get_center()).shift(2 * DOWN),
                        )
                    )
                else:
                    redRPSTransforms[i].append(
                        Transform(
                            redRPSDuplicates[i][2],
                            Circle(0.3, color=GREY)
                            .move_to(redRPS[i].get_center())
                            .shift(RIGHT + 2 * DOWN),
                        )
        ...
        lines.append([Line(LEFT, LEFT) for _ in range(3)])
        lines.append([Line(LEFT, LEFT) for _ in range(3)])
        lines[0][0].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[0].point_at_angle(225 * DEGREES),
                        redRPSDuplicates[0][0].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[0].point_at_angle(225 * DEGREES)[0]
                > redRPSDuplicates[0][0].point_at_angle(90 * DEGREES)[0]
                else None
            )
        )
        lines[0][1].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[0].point_at_angle(270 * DEGREES),
                        redRPSDuplicates[0][1].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[0].point_at_angle(270 * DEGREES)[1]
                > redRPSDuplicates[0][1].point_at_angle(90 * DEGREES)[1]
                else None
            )
        )
        lines[0][2].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[0].point_at_angle(315 * DEGREES),
                        redRPSDuplicates[0][2].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[0].point_at_angle(315 * DEGREES)[0]
                < redRPSDuplicates[0][2].point_at_angle(90 * DEGREES)[0]
                else None
            )
        )
        lines[1][0].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[1].point_at_angle(225 * DEGREES),
                        redRPSDuplicates[1][0].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[1].point_at_angle(225 * DEGREES)[0]
                > redRPSDuplicates[1][0].point_at_angle(90 * DEGREES)[0]
                else None
            )
        )
        lines[1][1].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[1].point_at_angle(270 * DEGREES),
                        redRPSDuplicates[1][1].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[1].point_at_angle(270 * DEGREES)[1]
                > redRPSDuplicates[1][1].point_at_angle(90 * DEGREES)[1]
                else None
            )
        )
        lines[1][2].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[1].point_at_angle(315 * DEGREES),
                        redRPSDuplicates[1][2].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[1].point_at_angle(315 * DEGREES)[0]
                < redRPSDuplicates[1][2].point_at_angle(90 * DEGREES)[0]
                else None
            )
        )
        lines[2][0].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[2].point_at_angle(225 * DEGREES),
                        redRPSDuplicates[2][0].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[2].point_at_angle(225 * DEGREES)[0]
                > redRPSDuplicates[2][0].point_at_angle(90 * DEGREES)[0]
                else None
            )
        )
        lines[2][1].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[2].point_at_angle(270 * DEGREES),
                        redRPSDuplicates[2][1].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[2].point_at_angle(270 * DEGREES)[1]
                > redRPSDuplicates[2][1].point_at_angle(90 * DEGREES)[1]
                else None
            )
        )
        lines[2][2].add_updater(
            lambda z: (
                z.become(
                    Line(
                        redRPS[2].point_at_angle(315 * DEGREES),
                        redRPSDuplicates[2][2].point_at_angle(90 * DEGREES),
                    )
                )
                if redRPS[2].point_at_angle(315 * DEGREES)[0]
                < redRPSDuplicates[2][2].point_at_angle(90 * DEGREES)[0]
                else None
            )
        )
        ...
        end_of_game_text = (
            Tex("End of Game", font_size=28)
            .move_to(redRPS[2].get_center())
            .shift(2.5 * RIGHT + 2 * DOWN)
        )

        rockVec = LEFT + 2 * DOWN
        scissorsVec = RIGHT + 2 * DOWN
        rockText1 = (
            Tex("Rock", font_size=20)
            .rotate(np.arctan(rockVec[1] / rockVec[0]))
            .move_to(lines[0][0].point_from_proportion(0.6) + 0.3 * UP)
        )
        paperText1 = (
            Tex("Paper", font_size=20)
            .rotate(-PI / 2)
            .move_to(lines[0][1].point_from_proportion(0.5) + 0.15 * RIGHT)
        )
        scissorsText1 = (
            Tex("Scissors", font_size=20)
            .rotate(np.arctan(scissorsVec[1] / scissorsVec[0]))
            .move_to(lines[0][2].point_from_proportion(0.6) + 0.3 * UP)
        )
        rockText2 = (
            Tex("Rock", font_size=20)
            .rotate(np.arctan(rockVec[1] / rockVec[0]))
            .move_to(lines[1][0].point_from_proportion(0.6) + 0.3 * UP)
        )
        paperText2 = (
            Tex("Paper", font_size=20)
            .rotate(-PI / 2)
            .move_to(lines[1][1].point_from_proportion(0.5) + 0.15 * RIGHT)
        )
        scissorsText2 = (
            Tex("Scissors", font_size=20)
            .rotate(np.arctan(scissorsVec[1] / scissorsVec[0]))
            .move_to(lines[1][2].point_from_proportion(0.6) + 0.3 * UP)
        )
        rockText3 = (
            Tex("Rock", font_size=20)
            .rotate(np.arctan(rockVec[1] / rockVec[0]))
            .move_to(lines[2][0].point_from_proportion(0.6) + 0.3 * UP)
        )
        paperText3 = (
            Tex("Paper", font_size=20)
            .rotate(-PI / 2)
            .move_to(lines[2][1].point_from_proportion(0.5) + 0.15 * RIGHT)
        )
        scissorsText3 = (
            Tex("Scissors", font_size=20)
            .rotate(np.arctan(scissorsVec[1] / scissorsVec[0]))
            .move_to(lines[2][2].point_from_proportion(0.6) + 0.3 * UP)
        )
        rockText1.add_updater(
            lambda z: z.move_to(lines[0][0].point_from_proportion(0.7) + 0.3 * UP)
        )
        paperText1.add_updater(
            lambda z: z.move_to(lines[0][1].point_from_proportion(0.5) + 0.15 * RIGHT)
        )
        scissorsText1.add_updater(
            lambda z: z.move_to(lines[0][2].point_from_proportion(0.7) + 0.3 * UP)
        )
        rockText2.add_updater(
            lambda z: z.move_to(lines[1][0].point_from_proportion(0.7) + 0.3 * UP)
        )
        paperText2.add_updater(
            lambda z: z.move_to(lines[1][1].point_from_proportion(0.5) + 0.15 * RIGHT)
        )
        scissorsText2.add_updater(
            lambda z: z.move_to(lines[1][2].point_from_proportion(0.7) + 0.3 * UP)
        )
        rockText3.add_updater(
            lambda z: z.move_to(lines[2][0].point_from_proportion(0.7) + 0.3 * UP)
        )
        paperText3.add_updater(
            lambda z: z.move_to(lines[2][1].point_from_proportion(0.5) + 0.15 * RIGHT)
        )
        scissorsText3.add_updater(
            lambda z: z.move_to(lines[2][2].point_from_proportion(0.7) + 0.3 * UP)
        )

        end_of_game_text.add_updater(
            lambda z: z.move_to(redRPS[2].get_center()).shift(2.5 * RIGHT + 2 * DOWN)
        )
        self.play(
            AnimationGroup(
                AnimationGroup(*flatRedRPSTransforms),
                AnimationGroup(
                    Write(end_of_game_text),
                    Write(rockText1),
                    Write(rockText2),
                    Write(rockText3),
                    Write(paperText1),
        ...
        self.play(
            FadeOut(rockText, paperText, scissorsText),
            VGroup(
                redRPSDuplicates[0][0],
                redRPSDuplicates[0][1],
                redRPSDuplicates[0][2],
                redRPS[0],
            ).animate.shift(redPaper.get_center() - redRPS[0].get_center()),
            VGroup(
                redRPSDuplicates[2][0],
                redRPSDuplicates[2][1],
                redRPSDuplicates[2][2],
                redRPS[2],
...
class bet(Scene):
    ...
    def construct(self):
        ...
        for i in range(60):
            ...
            lines.append(
                Line(
                    start_node.get_center() + 0.3 * unit_v,
                    target_nodes[i].get_center() - 0.05 * unit_v,
                    color=GRAY,
                )
            )
        Line(start_node)
        ...
        for i in range(10):
            ...
            lines2.append(
                Line(
                    start_node.get_center() + 0.3 * unit_v,
                    target_nodes_2[i].get_center() - 0.2 * unit_v,
                    color=GRAY,
                )
            )
...
class valueTemplate(Scene):
    def construct(self):
        ...
        pointer.add_updater(lambda m: m.next_to(number_line.n2p(tracker.get_value()), UP))
